...
RSpec.describe DailyReport, type: :model do
  describe 'titleのバリデーション' do
    context '適切な値のとき' do
      it '有効' do
        daily_report = build :daily_report, title: 'title'
    ...
    context 'titleがnilのとき' do
      it '無効' do
        daily_report = build :daily_report, title: nil
  ...
  describe '#learned_tags_str' do
    ...
    it 'learned_tagを持つとき","で区切って返す' do
      daily_report = build :daily_report, learned_tags: [
        build(:learned_tag, name: 'foo'),
        build(:learned_tag, name: 'bar')
      ]
